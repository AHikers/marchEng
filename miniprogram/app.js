    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const res = await wx.cloud.callContainer({
          "config": {
            "env": "prod-9gwglbkic70e2d42"
          },
          "path": obj.path,
          "header": {
            "X-WX-SERVICE": "express-g9o7"
          },
          "method": obj.method,
          "data": obj.data
        })
        // console.log('【node api调用成功】', res)
        // wx.hideLoading() // 取消加载
        if (res.data.code === 0) { // 如果返回code为0，则业务正常
          resolve(res.data)
        } else { // 如果返回code不为0，则证明有点问题
          let tips = res.data.msg || '服务调用出现未知问题，请重新尝试！' // 提示文字如果没回传，默认提示
